...
# ==============================================
# MODEL LOADING - more robust version
# ==============================================
@st.cache_resource
    ...
    for model_path in possible_paths:
        if os.path.exists(model_path):
            try:
                ...
                st.warning(f"Found file but failed to load: {model_path}\nError: {str(e)}")
    
    st.error("❌ Model file 'model_fruit.h5' not found in any expected location!")
    st.info("Make sure model_fruit.h5 is in the **same folder** as this script (app.py/main.py)")
    return None, None

model, loaded_model_path = load_fruit_model()
...
CLASS_NAMES = [
    'DarkRottenBanana_3', 'GreenApple_1', 'GreenOrange_1', 'OrangeOrange_2',
    'RedApple_2', 'RottenApple_3', 'RottenBanana_2', 'RottenOrange_3', 'YellowBanana_1'
]
...
def get_fruit_info(label):
    ...
    if 'Banana' in label:
        if 'Yellow' in label:          return "Banana", 2, "Ripe (Yellow)"
        if 'DarkRotten' in label:      return "Banana", 3, "Rotten (Dark Brown-Black)"
        if 'Rotten' in label:          return "Banana", 3, "Overripe/Rotten (Brown)"
        return "Banana", 1, "Unripe (Green)"
    
    if 'Apple' in label:
        if 'Red' in label:             return "Apple", 2, "Ripe (Red)"
        if 'Rotten' in label:          return "Apple", 3, "Rotten"
        return "Apple", 1, "Unripe (Green)"
    
    if 'Orange' in label:
        if 'OrangeOrange' in label:    return "Orange", 2, "Ripe (Orange)"
        if 'Rotten' in label:          return "Orange", 3, "Rotten"
        return "Orange", 1, "Unripe (Green)"
...
if st.session_state.page == "home":
    st.markdown('<div class="title-big">🍎🍌🍊 Fruit Ripeness Classifier</div>', unsafe_allow_html=True)
    st.markdown('<div class="subtitle">AI-Powered Ripeness Detection for Fruits</div>', unsafe_allow_html=True)
    
    col1, col2, col3 = st.columns([1, 3, 1])
    with col2:
        if st.button("CLASSIFY FRUIT", key="btn_simple", use_container_width=True, help="Quick result"):
            st.session_state.page = "classify"
            st.rerun()
            
        if st.button("PREDICT RIPENESS (FULL)", key="btn_full", use_container_width=True, help="Detailed + reference images"):
...
# ==============================================
# SIMPLE CLASSIFY PAGE
# ==============================================
elif st.session_state.page == "classify":
    st.title("Classify Fruit Ripeness")
...
# ==============================================
# FULL PREDICTION PAGE (with reference images + debug)
# ==============================================
    ...
    if uploaded_file is not None:
        ...
        if model is None:
            ...
            # Reference images
            st.subheader("Ripeness Stages Reference")
            if fruit == "Banana":
                cols = st.columns(3)
                with cols[0]: 
                ...
                with cols[1]: 
                    ...
                    if p: 
                        cap = "Grade 2 – Ripe" + (" ← PREDICTED" if grade == 2 else "")
                ...
                with cols[0]: 
                    ...
                    if p: st.image(p, caption="Grade 1 – Unripe")
                with cols[1]: 
                    p = get_first_image("FreshOrange_2")  # Note: your class is OrangeOrange_2
                    if p: 
                        cap = "Grade 2 – Ripe" + (" ← PREDICTED" if grade == 2 else "")
                        st.image(p, caption=cap)
                with cols[2]: 
